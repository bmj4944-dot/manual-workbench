...
import { SAMPLE_PDFS } from "@/lib/sample-data";
...
export function PdfViewer({ nodeId }: Props) {
  const meta = SAMPLE_PDFS[nodeId] ?? { title: "첨부 PDF", pages: 1 };
  ...
  useEffect(() => {
    if (mode === "upload") renderPage(page);
  ...
  const onFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : "PDF 로드 실패");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-4 px-10 py-10 text-zinc-700">
        ...
        <p className="text-[14px] leading-relaxed">
          ...
          파일을 드롭하면 pdf.js로 실제 페이지가 렌더됩니다.
        </p>
